...
# This used to force CAJETA_CAPTURED_BORROW=warn, because
# CAJETA_ERROR_CAPTURED_BORROW_PARAM rejected GzipCompressChannel keeping its
# source reader in a field. It is gone because it suppresses nothing any more:
# the constructor now stores `this.src #= src`, which is the container spelling
# the diagnostic itself names, so the tree builds clean with the check at full
# strength. A standing suppression that catches nothing only hides the next
# real one.
#
# Measured on cajeta v0.29.0 with .cajeta/cache cleared before each build.
# Unset, and at an explicit `error`, the library and the suite compile with no
# CAPTURED_BORROW diagnostic at all, and so does v0.28.0. The check is not
# merely quiet: a three-file probe that stores a plain parameter in a field
# fails on the same binary with CAJETA_ERROR_CAPTURED_BORROW_PARAM.
